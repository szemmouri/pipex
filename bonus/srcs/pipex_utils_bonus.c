...
/*   Updated: 2025/02/25 10:45:01 by szemmour         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	if (ft_strncmp(in_paht, "here_doc", ft_strlen(in_paht)) == 0)
	{
		fd->in = open("/tmp/here_doc", O_RDONLY);
		fd->out = open(out_path, O_CREAT | O_WRONLY | O_APPEND, 0644);
	}
	else
	{
		fd->in = open(in_paht, O_RDONLY);
		fd->out = open(out_path, O_CREAT | O_WRONLY | O_TRUNC, 0644);
	}
	if (fd->out == -1)
	{
		perror("open");
